---
title: "Introduction to catchEmAll"
author: "Trent Henderson"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to catchEmAll}
...
The catchEmAll R package provides access to an open-source implementation of a range of sets of time-series feature statistics coded in C and C++ for computational efficiency and scalability. The package name is a nod to the [Pokemon theme song](https://www.youtube.com/watch?v=MpaHR-V_R-o) that we all know and love - where instead of catching lots of Pokemon, the package provides lots of sets of CAnonical Time series CHaracteristics (catch) operations.

## Core calculation functions

Before we look at the available functions in the package, let's first pull some time-series data from the [Empirical 1000](https://figshare.com/articles/dataset/1000_Empirical_Time_series/5436136) dataset. This section of code isn't directly needed for the package.
...
There are three core functions that automate the calculation of many feature statistics at once:

* `catch_all`
* `catch22_all`
* `catchaMouse16_all`

These are demonstrated using a loop for step clarity, but should be vectorised in practice due to R's computational nature. Note you can interchange `catch_all` for `catch22_all` or `catchaMouse16_all` if you just want to calculate the features in those sets.
...
The `catch_all` function automatically fits *every* feature available in the package. The `catch22_all` and `catchaMouse16` functions fit the feature statistics contained within their sets. The full feature list can be accessed via a dataset that comes stock with the package:

```{r, message = FALSE, warning = FALSE}
View(feature_list)
```

If you do not want to or need to run the entire sets included in `catch_all`, `catch22_all`, `catchaMouse16_all`, you can also access the individual feature calculations as their unique function names, for example `SC_FluctAnal_2_rsrangefit_50_1_logi_prop_r1`.
